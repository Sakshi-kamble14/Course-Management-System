...
async function chat({ question, currentCourse }) {
  ...
  const response = await fetch(process.env.MISTRAL_API_URL || 'https://api.mistral.ai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.2,
      messages: [
        { role: 'system', content: 'You are EduManage Course Assistant. Use only the supplied active course catalog. Make recommendations from it, explain descriptions in simple words, and discuss the student current course when supplied. Do not invent prerequisites, lessons, fees, dates, outcomes, or courses. If the catalog does not support an answer, say that clearly. Never enroll the student or imply that you did. Keep answers concise and practical.' },
